...
export default function DirectSaleCTA({ recommendation, equipment, contactName, contactCompany, contactPhone, callId }) {
  ...
  if (status === 'sent') {
    return (
      <div className="px-4 py-3 rounded text-center" style={{ background: 'var(--cockpit-live-bg)', border: '1px solid var(--cockpit-live-border)' }}>
        <div className="text-sm font-semibold" style={{ color: 'var(--cockpit-live-500)' }}>Quote request sent</div>
  ...
  if (status === 'audit-sent') {
    return (
      <div className="px-4 py-3 rounded text-center" style={{ background: 'var(--cockpit-live-bg)', border: '1px solid var(--cockpit-live-border)' }}>
        <div className="text-sm font-semibold" style={{ color: 'var(--cockpit-live-500)' }}>Audit scheduled</div>
  ...
  return (
    <div className="space-y-2">
      {/* Direct sale badge */}
      <div
        className="text-[11px] font-semibold tracking-[1.5px] text-center py-1.5 rounded"
      ...
      >
        <div className="text-sm font-semibold">Schedule 10-Min Compressed Air Audit</div>
      ...
      {!emailForm ? (
        ...
        >
          <div className="text-sm font-semibold">Send Custom Quote via Email</div>
          <div className="text-[11px] mt-0.5" style={{ color: 'var(--cockpit-text-muted)' }}>We'll follow up with detailed pricing</div>
        </button>
      ) : (
        <form onSubmit={handleQuoteSubmit} className="px-4 py-3 rounded space-y-2" style={{ background: 'var(--cockpit-live-bg)', border: '1px solid var(--cockpit-live-border)' }}>
          <input
            type="email"
            required
            value={email}
